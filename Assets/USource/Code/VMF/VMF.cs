        ...
        public static GameObject CreateFromVMF(System.IO.Stream stream, ImportMode importMode = ImportMode.CreateAndCache)
        {
            ...
            CSGBrush Create(VBlock block, out Layer layer)
            {
                ...
                foreach (VBlock side in block.Body.Where((IVNode node) => { return node.Name == "side"; }))
                {
                    ...
                    USource.ResourceManager.GetUnityObject(location, out UnityEngine.Material resourceMaterial, importMode, true);
                ...
                if (disable == false && brush.Shape.TexGenFlags.All((RealtimeCSG.Legacy.TexGenFlags flags) =>
                {
                    return
            ...
            {
                UnityEditor.EditorUtility.DisplayProgressBar($"Importing", "Creating entities", (float)entityCounter / entityCount);
                entityCounter++;
                if (node is VBlock block && node.Name == "entity")
                {
                    ...
                    if (className.Contains("prop_"))
                    {
                        ...
                        if (USource.ResourceManager.GetUnityObject(new Location(modelName, Location.Type.Source), out GameObject resource, importMode, true))
                        {
                            if (resource == null)
                            {
                            ...
                            if (importMode == ImportMode.AssetDatabase)
                                gameObject = UnityEditor.PrefabUtility.InstantiatePrefab(resource, vmfGO.transform) as GameObject;
#endif
                            if (importMode == ImportMode.CreateAndCache)
                                gameObject = GameObject.Instantiate(resource, vmfGO.transform);
                    ...
                    else if (className.Contains("light"))
                    {
                        ...
                        light.transform.parent = vmfGO.transform;
                    ...
                    else if (className == "env_cubemap")
                    {
                        ...
                        cubemap.transform.parent = vmfGO.transform;
                    }
                }
            }
