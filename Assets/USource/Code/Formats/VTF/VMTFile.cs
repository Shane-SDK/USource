...
namespace USource.Formats.VTF
{
    public class VMTFile
    {
        ...
        public bool ContainsParma(string parma)
        {
        ...
        public float GetSingle(string parma)
        {
            return GetParma(parma).ToSingle();//float.Parse(Items[Data]);
        }
        public int GetInteger(string parma)
        {
            return GetParma(parma).ToInt32();//float.Parse(Items[Data]);
        }
